...
class ALIGN BitArray : public std::array<uint8_t, N> {
    ...
    /** Bit-wise and. */
      ...
      #if defined(__AVX2__) && defined(__AVX__)
			for ( ; i+32 <= N ; i += 32 ) {
				auto x = _mm256_load_si256((__m256i*)(&std::array<uint8_t, N>::at(i)));
				auto y = _mm256_load_si256((__m256i*)(&rhs.at(i)));
				x = _mm256_and_si256(x,y);
				_mm256_store_si256((__m256i*)(&std::array<uint8_t, N>::at(i)), x);
			}
      #elif defined(__AVX__)
			for ( ; i+16 <= N ; i += 16 ) {
				auto x = _mm_load_si128((__m128i*)(&std::array<uint8_t, N>::at(i)));
				auto y = _mm_load_si128((__m128i*)(&rhs.at(i)));
				x = _mm_and_si128(x,y);
				_mm_store_si128((__m128i*)(&std::array<uint8_t, N>::at(i)), x);
			}
      #endif
    ...
    BitArray operator~() const {
		...
		BitArray<N+M> operator+(const BitArray<M>& rhs) const {
			BitArray<N+M> ret;
			std::copy(&(*this)[0], &(*this)[N], &(ret[0]));
			std::copy(&rhs[0], &rhs[M], &ret[N]);
